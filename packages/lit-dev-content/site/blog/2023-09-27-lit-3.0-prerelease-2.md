...
export class SignalExample extends SignalWatcher(LitElement) {
  ...
  private _onClick() {
    // Change the signal value causes the element to re-render!
...
You should also update dependencies on our other packages if you use those so that they pick up the Lit prerelease as well.
...
Remember that Lit 2.x and Lit 3.x are interoperable, so even if some of your dependencies are using Lit 2.x you can try the Lit 3.0 prerelease on your components!
