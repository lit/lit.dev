...
`unsafeSVG` creates elements in the SVG namespace, so it's for use inside of `<svg>` tags or inside of lit-html {% api-lit-v2 "svg" "svg" %} templates. If the input contains an `<svg>` tag itself, continue to use `unsafeHTML`.
