...
While Lit ships with a number of built-in directives like [`repeat()`](/docs/templates/built-in-directives/#repeat) and [`cache()`](/docs/templates/built-in-directives/#cache), users can author their own custom directives. To create a directive:
