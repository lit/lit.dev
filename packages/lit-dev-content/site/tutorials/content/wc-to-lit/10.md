...
Now that you have gotten rid of the `<template>` element in `index.html`, refactor the code to instead call the newly-defined `render` method. You can start by leveraging lit's event listener binding syntax:
...
* were able to remove the dom update logic from the `rating` setter
