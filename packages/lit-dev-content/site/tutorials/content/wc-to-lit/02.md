...
<p>It is considered bad practice to do DOM manipulations in the constructor, because DOM manipulations can slow down initial load times.</p>

<p>Additionally, the browser will throw an error if a custom element definition that manipulates the DOM in the <code>constructor</code> is loaded while the browser is still parsing the document because the parser does not know how to handle changes to the DOM mid-parse. This is common in inline scripts or in a <code>&lt;script></code> that is not <code>type="module"</code></p>
</aside>
