...
Using `innerHTML` and template literal strings with no sanitization may cause security issues with script injection. Before `<template>` was aded to the browser, in the past, it was common to use `DocumentFragments`, but these also come with other issues such as images loading and scripts running when the templates are defined but not rendered, as well as introducing obstacles for reusability.
